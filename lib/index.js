...
var runServer = function(options) {
  ...
  var serverPort = options.serverPort || 8080;
  var localSettingsHook = options.localSettingsHook || (function() {});
  ...
  var handlePhp = function(phpfile, pathinfo, req, res) {
    php.request({
      ...
      serverInitFunc: function(server) {
        server.SERVER_NAME = serverName; // XXX
      ...
      context: {
        mwHook: function(globals) {
          ...
          // But when we can, we'd like to set the serverName & port.
          localSettingsHook(globals); // Allow end user to do additional config.
        },
      },
    }).catch(function(e) {
